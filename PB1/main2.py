...
def greedy2(N_vid, N_endpoint, N_requests, N_caches, caches_sizes, videoSizes, caches, endpointData, requests):
    ...
    pour chaque video on regarde les caches accessibles par les endpoints qui la demandent
    on trie les caches par nombre de fois accessibles par les endpoints qui demandent la video
    """
    ...
    # for video_id in video_list_sorted:
    #     for cache_id, _ in videos_caches[video_id]:
    #         if videoSizes[video_id] <= caches_sizes[cache_id] and video_id not in caches[cache_id]:
    #             caches[cache_id].append(video_id)
    #             caches_sizes[cache_id] -= videoSizes[video_id]
    #             break  # Move to the next video after placing it in a cache
...
def main(args):
    ...
    # preprocess_requests(N_vid, N_endpoint, N_request, N_cache, requests, endpoints)
    ...
    print(f"[{time_taken:.4f}s] Greedy : {greedyCost}")
    print(f"Improvement: {base_cost - greedyCost} ({(base_cost - greedyCost) / base_cost * 100:.2f}%)")

    # ========================== GREEDY2 ==========================
    print("----- Greedy2 -----")
    caches2 = [[] for _ in range(N_cache)]  # la liste des videos stockés dans chaque cache
    caches_sizes2 = [cache_size] * N_cache  # la taille restante de chaque
    current_time = time.time()
    greedy2(N_vid, N_endpoint, N_request, N_cache, caches_sizes2, video_sizes, caches2, endpoints, requests)
    time_taken = time.time() - current_time

    current_time = time.time()
    greedy2Cost = compute_cost(caches2, endpoints, requests)
    print(f"[{time_taken:.4f}s] Greedy2 : {greedy2Cost}")
    print(f"Improvement: {base_cost - greedy2Cost} ({(base_cost - greedy2Cost) / base_cost * 100:.2f}%)")
    ...
    print(f"[{time_taken:.4f}s] Local search : {localSearchCost}")
    print(f"Improvement: {base_cost - localSearchCost} ({(base_cost - localSearchCost) / base_cost * 100:.2f}%)")

    # ========================== LOCAL SEARCH ON GREEDY2 ==========================
    print("------ Local search on greedy2 ------")
    current_time = time.time()
    local_search(N_vid, N_endpoint, N_request, N_cache, caches_sizes2, video_sizes, caches2, endpoints, requests, iteration=10, previous_moves=None, nbCaches=50, nbVideos=100, supp=True)
    time_taken = time.time() - current_time

    localSearchCost2 = compute_cost(caches2, endpoints, requests)
    print(f"[{time_taken:.4f}s] Local search on greedy2 : {localSearchCost2}")
    print(f"Improvement: {base_cost - localSearchCost2} ({(base_cost - localSearchCost2) / base_cost * 100:.2f}%)")
    ...
    results = {
        ...
        "Greedy": greedyCost,
        "Greedy2": greedy2Cost,
        "LS (greedy)": localSearchCost,
        "LS (greedy2)": localSearchCost2,
    }
